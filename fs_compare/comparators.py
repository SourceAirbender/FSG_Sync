...
def compare_parents(db, gr_person: Person, fs_person) -> List[Tuple]:
    ...
    if len(fs_person._parents) > 0:
        ...
        for fsid in parent_ids:
            fs2 = gedcomx_v1.Person._index.get(fsid) or gedcomx_v1.Person()
...
def compare_spouse_notes(db, gr_person: Person, fs_person) -> List[Tuple]:
        ...
        if family:
            ...
            if FSG_Sync.FSG_Sync.fs_Tree:
                fs_spouse = (
                    ...
                    or gedcomx_v1.Person()
                )
            else:
                fs_spouse = gedcomx_v1.Person()
    ...
    for couple in fs_spouses:
        ...
        fs_name = fs_spouse.preferred_name() if fs_spouse else gedcomx_v1.Name()
        res.append(
            (
                color,
                _trans.gettext("Spouse"),
...
def compare_spouses(db, gr_person: Person, fs_person) -> List[Tuple]:
    # Add spouse AND family event comparisons to result list
    res: List[Tuple] = []
    fs_spouses = fs_person._spouses.copy()
    ...
    for family_handle in gr_person.get_family_handle_list():
        ...
        if family:
            ...
            # Family-level events (e.g., Marriage)
            if fs_pair:
            ...
            for eventref in family.get_event_ref_list():
                ...
                gr_value = (
                    gr_desc
                    if gr_place == ""
                    else (gr_desc + " from gedcomx_v1.xml import parse_xml " + gr_place)
                )
                ...
                fs_value = (
                    fs_desc
                    if fs_place == ""
                    else (fs_desc + " from gedcomx_v1.xml import parse_xml " + fs_place)
                )
            ...
            for triple in fs_children:
                if (
                    ...
                    fs_name = fs_child.preferred_name() if fs_child else gedcomx_v1.Name()
                    res.append(
                        (
                            color,
                            "    " + _trans.gettext("Child"),
...
def compare_other_facts(db, person: Person, fs_person) -> List[list]:
    ...
    for gr_ref in gr_facts:
        ...
        # Otherwise, match by same tag and date
