...
def fs_date_to_gramps_date(fs_date) -> Optional[Date]:
    ...
    year = month = day = 0
    year2 = month2 = day2 = 0  # for ranges
    ...
    if formal:
        ...
        if getattr(formal, "is_range", False):
            # If only one bound is present, mark BEFORE/AFTER
...
def gramps_date_to_formal(date_obj: Date) -> str:
    """Convert a Gramps :class:`Date` to a GEDCOM X "formal" date string.
    ...
    if gd.dateval[Date._POS_DAY] > 0:
        ...
        # No numeric value; fall back to text
        return gd.text or ""
